...
create index professional_business_search_idx
  on public.professional_profiles using gin (business_name extensions.gin_trgm_ops);
create index professional_headline_search_idx
  on public.professional_profiles using gin (headline extensions.gin_trgm_ops);
create index professional_city_search_idx
  on public.professional_profiles using gin (city extensions.gin_trgm_ops);
create index professional_service_title_search_idx
  on public.professional_services using gin (title extensions.gin_trgm_ops)
  where is_active;
create index professional_verified_name_idx
  on public.professional_profiles (business_name, user_id)
...
$$;
revoke all on function public.search_professionals(text, uuid, text, integer, integer) from public, anon;
grant execute on function public.search_professionals(text, uuid, text, integer, integer) to authenticated;
